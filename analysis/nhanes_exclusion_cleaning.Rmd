...
bmd_fert_fem_mega_data <- drop_na(fert_fem_mega_data, "DXXLSBMD")
```

## Add PBMDA column for AFB before 27yo vs at/after 27yo
```{r}
...
# if birth occurred before participant was 27, coded 1, if occurred on or after participant was 27, coded 0
bmd_fert_fem_mega_data <- bmd_fert_fem_mega_data %>%
  mutate(PBMDA = ifelse(RHD180 < 27, 1, 0 ))
```


# Analysis

## Running univariate linear regression with BMD (DXXLSBMD) vs AFB (RHD180) 
```{r}
# run an univariate linear regression where independent/x = AFB, dependent/y = BMD
ULM_1 <- lm(DXXLSBMD ~ RHD180, data = bmd_fert_fem_mega_data)

# visualize ULM_1 via scatter plot
ULM_1_graph <- ggplot(ULM_1, aes(x = RHD180, 
            y = DXXLSBMD)) + geom_point(alpha = 0.5) + scale_x_continuous("AFB (yrs)", breaks = seq(0, 40, by = 2)) + scale_y_continuous("Bone Mineral Density (g/cm^2)", breaks = seq(0, 2, by = 0.1)) + annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1.1, ymax = 1.2, fill = "lightgreen", alpha = 0.2)
  
# view the plot
print(ULM_1_graph)
```

## Running univariate linear regression with BMD (DXXLSBMD) vs binary AFB (PBMDA)
```{r}
# run an univariate linear regression where x = AFB before PBMDA OR at/after PBMDA, y = BMD
ULM_2 <- lm(DXXLSBMD ~ PBMDA, data = bmd_fert_fem_mega_data)

# visualize ULM_2 via beeswarm plot
ULM_2_graph <- ggplot(ULM_2, aes(x = PBMDA, y = DXXLSBMD)) +
  geom_beeswarm() + scale_x_continuous("AFB before OR on/after PBMDA (no/yes)", breaks=seq(0,1,by=1)) + scale_y_continuous("Bone Mineral Density (g/cm^2)", breaks = seq(0, 2, by = 0.1)) + annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1.1, ymax = 1.2, fill = "lightgreen", alpha = 0.2)
