...
```{r}
# run a univariate linear regression where independent/x = AFB, dependent/y = BMD
...
***need to know what kind of plot to do with this... (1/30)
```{r}
# run a univariate linear regression where x = AFB before PBMDA OR at/after PBMDA, y = BMD
ULM_2 <- lm(DXXLSBMD ~ PBMDA, data = bmd_fert_fem_mega_data)

# visualize ULM_2 via scatter plot
ULM_2_graph <- ggplot(ULM_2, aes(x=PBMDA, 
            y = DXXLSBMD)) + geom_point()
