...
### Simple linear regression: BMD v AGE (at time of scan)
```{r}
lm <-
  lm(BMD ~ AGE, data = df)
summary(lm)
...
### Linear mixed-effects model 1: BMD vs AFB, AGE, RETH
```{r}
# classic linear model
lm2 <- 
  lmer(BMD ~ AFB + AGE + RETH + (1 | SEQN), data = df)
summary(lm2)

# optimal random structure
lmm1 <- lmer(BMD ~ AFB + AGE + RETH, data = df, random = ~1 | SEQN) 
summary(lmm1)
