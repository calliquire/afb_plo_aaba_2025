...
```{r}
# convert variables to factors to run TukeyHSD later on
df$AFB <- as.factor(df$AFB)
df$AGE <- as.factor(df$AGE)

# simple multivariate regression
lm1 <- aov(BMD ~ AFB + AGE, data = df)
summary(lm1)

# added other potential variables
lm2 <- aov(BMD ~ AFB + AGE + RETH, data = df)
summary(lm2)

# testing interaction effects btw AFB and AGE
lm3 <- aov(BMD ~ AFB * AGE + RETH, data = df)
summary(lm3)

# non-linear effect of AGE (as a polynomial term)
lm4 <- aov(BMD ~ AFB + AGE + I(AGE^2) + RETH, data = df)
summary(lm4)

# find best fit model (AIC test for model fit)
model.set <- list(lm1, lm2, lm3, lm4)
model.names <- c("lm1", "lm2", "lm3", "lm4")
