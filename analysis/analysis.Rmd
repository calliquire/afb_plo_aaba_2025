...
plot5 <-
  ...
  labs(title = "BMD vs Age at Time of Scan by AFB", x = "Age at Time of Scan (yrs)", y = "Femoral Neck BMD (g/cm^2)", color = "Age at First Birth (yrs)") +
...
plot6 <- 
  ...
  scale_color_brewer(palette = "Dark2", breaks = c('Nulliparous', '1-3', '4-6', '7-9', '10+')) +
  scale_x_continuous(breaks = seq(0, 70, by = 5)) +
  labs(title = "Bone Mineral Density vs Age at Time of Scan by Parity", x = "Age at Time of Scan (yrs)", y = "Femoral Neck BMD (g/cm^2)", color = "Parity") +
  geom_smooth(method = "lm", se = F)
print(plot6)
...
# remove noise of middle AFBs
df3 = df %>%
    filter(BINS != "19-26")

# restrict participants to 30-55
df3 <- subset(df3, !(AGE %in% c(0:34)))
...
plot8 <-
  ...
  scale_fill_brewer(palette = "Set2", breaks=c('</= 18', '>/= 27')) +
  scale_x_discrete(limits = c('1-3', '4-6', '7-9', '10+')) +
  geom_jitter(size = 0.4, alpha = 0.4) +
  labs(title = "Bone Mineral Density vs Parity grouped by Age at First Birth", y =  "Femoral Neck BMD (g/cm^2)", x = "Parity (live births/person)", fill = "AFB")
print(plot8)
