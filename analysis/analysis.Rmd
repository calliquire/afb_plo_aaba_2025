...
```{r}
# combine data sets for DEMO, RHQ, and DXX
...
## Merge into one main data set
```{r}
# merging DEMO and RHQ
predata <- left_join(DEMO, RHQ, by = "SEQN") 

# merging data (includes DEMO, RHQ) w/ DXX
data <- left_join(predata, DXX, by = "SEQN")
```

## Combine same variables into single columns if needed
```{r}
# want to combine any variables that have two different variable names (e.g., RHD180 = RHQ180)
# define separate columns for diplyr
RHQ180 <- c(seq(12, 60, by=1), "777", "999")
RHD180 <- c(seq(12, 60, by=1), "777", "999")

# use coalesce() to combine and remove alternating N/A's
coalesce(RHQ180, RHD180)
