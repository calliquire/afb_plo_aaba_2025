...
lm6 <- 
  lm(BMD ~ PAR + AGE, data = df)
summary(lm6)

lm7 <-
  lm(BMD ~ PAR + AGE + AFB, data = df)
summary(lm7)

lm8 <-
  lm(BMD ~ PAR + AGE + AFB + RETH, data = df)
summary(lm8)
...
lm10 <-
  lm(BMD ~ PAR + I(AGE^2) + AFB + RETH, data = df)
summary(lm10)
...
# Plots
```{r}
# visualize BMD vs PAR (by age at time of scan)
plot6 <- 
  ggplot(df, aes(x = PAR, y = BMD, fill = AGE)) +
  geom_point() +
  scale_fill_brewer(palette = "Dark2") +
  labs(title = "Bone Mineral Density vs Parity", x = "Parity (live births/person)", y = "Femoral Neck BMD (g/cm^2)") +
