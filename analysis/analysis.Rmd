...
### Exploratory analysis: linear regressions, non linear regressions
```{r}
# convert variables to factors to run Tukey HSD later on?
...
plot1 <- 
  ...
  geom_point(alpha = 0.5) + scale_x_continuous(breaks = seq(0, 70, by = 5)) + labs(title = "Bone Mineral Density vs Age at Time of Scan", y = "Femoral Neck BMD (g/cm^2)", x = "Age at Time of Scan (yrs)")
print(plot1)
...
plot3 <-
  ...
  geom_boxplot() +
  labs(title = "Bone Mineral Density vs Binned Age at First Birth", x = "Age at First Birth (yrs)", y = "Femoral Neck BMD (g/cm^2)", fill = "Binned AFB") +
  scale_x_discrete(limits = c("</= 18","19-26",">/= 27")) +
  scale_fill_brewer(breaks=c('</= 18', '19-26', '>/= 27'))
print(plot3)
